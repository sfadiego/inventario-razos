...
export const FormDevolucion = ({ isOpen, onClose, ventaId }: IFormDevolucion) => {
  ...
  return (
    <Modal
      title="Devolución"
      subtitle={!devolucionCreada ? 'Generar Devolución' : 'Actualizar Devolución'}
      isOpen={isOpen}
      onClose={handleClose}
    ...
    >
      <div className="grid grid-cols-12 gap-3">
        ...
        {productosDevolucion && productosDevolucion.length > 0 && (
          <ProductosDevolucion productosDevolucion={productosDevolucion} hasDevolucion={devolucionCreada} handleRemove={handleRemove} />
        )}
        <div className="col-span-12 mt-3 flex justify-end gap-2">
          ...
          <Button
            disabled={!enableBtnPayload || devolucionCancelada}
            className="col-span-12 lg:col-span-6"
            size="md"
            variant={`${!devolucionCreada ? 'primary' : 'warning'}`}
            type={ButtonTypeEnum.Button}
            onClick={handleDevolucion}
          >
            <HandCoins /> {!devolucionCreada ? 'Generar Devolución' : 'Cancelar Devolución'}
          </Button>
        </div>
      </div>
    </Modal>
